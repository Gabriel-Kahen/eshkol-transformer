...
python3 - "${evidence}/runtime.stdout" "${evidence}/nonfinite.stdout" <<'PY'
from pathlib import Path
import sys
source, destination = map(Path, sys.argv[1:])
lines = source.read_text().splitlines()
for i, line in enumerate(lines):
    if line.startswith('TR3-STEP-OBS 0 '):
        fields = line.split()
        fields[2] = str(0x7fc00000)
        lines[i] = ' '.join(fields)
        break
else:
    raise SystemExit('missing first objective')
...
printf 'TR3 step leaf evidence: %s\n' "${evidence}"
printf 'TR3 step leaf seal: %s\n' \
