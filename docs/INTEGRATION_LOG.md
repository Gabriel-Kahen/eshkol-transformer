...
- **Pin:** the isolated adoption candidate changes the active toolchain and serialized
  ...
  `https://github.com/Gabriel-Kahen/eshkol.git@81298b4a9608fb92eb6f351a2eabd8392da7d9ef`.
...
- **Compatibility:** the compiler version and 71-byte identity layout are unchanged.
  New C2 saves carry the new exact identity. Checkpoints with the immediately
  replaced `222cad3aac68ddf48d09c1cdf322fa4c4e7b8296` identity and the older
...
- **Evidence boundary:** prior component evidence remains tied to its recorded pin.
  A fresh network-disabled Ubuntu 22.04 / LLVM-Clang 21.1.8 build used container
  image digest
  `sha256:f31d1db76958339e6ebd2a2f667052cdb85aeb5229914ffb10ac4fcdc6db22e6`
  and the exact source commit above. Focused adoption evidence passed B0 41/41;
  C2 ordinary and ASan/UBSan/LSan format and codec exact-byte gates, including both
  historical-identity inspect/load negatives; the full private C2 save gate with four
  independent exact-byte images, strict Clang/GCC/C++, deterministic AOT artifacts,
  failpoints, sanitizers, and exact closure; the P1 registry IR/runtime proof with
  checked success/failure control flow and canonical post-barrier readback; and
  E3-METRICS exact package closure, 70,231 native checks, 135 fenv fault checks,
  102 exact-f32 Decimal inputs at maximum 0 ULP,
  22 killed mutants with five equivalent controls, two fresh deterministic private
  AOT builds, and ASan/UBSan/LSan with leak detection. The pinned Python 3.14.6 /
  PyTorch 2.13.0+cpu oracle regenerated byte-identical fixture and provenance files.
  Compiler-generated package manifests now admit the checked barrier plus the two
  emergency runtime entries and exclude the superseded unchecked barrier.
- **Disposition:** candidate only. The separate #117/#121 guard-order repairs must
  join before final aggregate evidence, and no full supported CI, merge, acceptance,
  or roadmap completion is claimed by this focused adoption gate.
