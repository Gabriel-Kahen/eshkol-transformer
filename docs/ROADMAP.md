...
| TR3 | Trainer state machine and exact resume | M3, L2, O2, D2, C2, E3 | #116 [private objective bridge integration candidate](TR3B_OBJECTIVE_BRIDGE.md), imported byte-for-byte from independently approved exact source `071abb72` (all six final review findings resolved): genuine D2→L2→L3S→M3-VJP compiled path, fourteen-gradient numerical/metadata/reset and failure-atomicity evidence, exact lifetime accounting, and ASan/UBSan+LSan local compatibility evidence on unsupported CachyOS/LLVM 22.1.6. Supported focused and full CI remain pending; full trainer, optimizer/checkpoint transactions, overfit and interrupted/resumed equivalence remain open | review |
