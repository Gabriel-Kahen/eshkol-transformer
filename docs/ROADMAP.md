...
| C2 | [Detached full training-state checkpoint schema](C2_TRAINING_STATE.md) | C1, D2, O2, X1, K2 | K2 and all component prerequisites are merged. Private parser/reader/inspect, exact-bit staging, canonical deep-owner lifetime, staged LOAD/reconstruction, and atomic SAVE pass. The source-composed public candidate has the exact 81-global/75-export/81-string boundary, authenticated policy/metadata carriers, fresh-cache lifecycle/isolation/determinism gates, flat 1,024/8,192 policy/metadata root retention, and poison/IR publication proof. Two final-source public 64-tensor LOAD/SAVE/release runs measured 519,028 and 520,656 KiB below the unchanged 524,288 KiB ceiling; exact per-tensor K2 admission, every injected failure ordinal, unsupported-rank and alias-shape cleanup, affected C2 regressions, and independent lifetime/numerical/packaging review pass locally. Supported Ubuntu 22.04/LLVM 21 blocking and exhaustive CI plus final integration acceptance remain outstanding. Component continuation and atomic file publication only; TR3 retains joint live restore/full trajectory and G3 generation proof | active |
