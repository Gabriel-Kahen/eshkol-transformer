...
The separately owned [issue101 shared contract](E3_SHARED_CALL_CONTRACT.md),
accepted and merged through PR #102 at `2ac0b5bc5a385e79700f46bf899bab841f33d1cb`,
supplies exactly
...
G3 imports. Its implementation is separately authorized and still a runtime
prerequisite; this proposal does not create or redefine it. This branch incorporates
that merged main, including the accepted19-suite/27-command CI union, without
changing upstream production files or numerical gates.
...
and `e3-frame-d2-record-internal/1`. The D2 companion additionally fixes
`e3-d2-release-owned!/2(frame,owned-cell)` for the region-local batch cell. Numeric selectors are closed role0..20 and
...
Closed-frame rejection mapping, before any destination/readiness write:

| E3-local condition | Status/code in domain0 |
|---|---|
| null/foreign unregistered frame or model/destination identity |1/1|
| enrolled dead frame, invalid lifecycle or wrong next phase |2/2|
| role or counter selector outside its fixed range |1/2|
| native/source model already occupied at idle admission |2/7|
| wrong exact rank/extents/byte length or input/target index |3/3|
| authentic unsupported model/profile; stage dtype/device/layout/offset outside fixed schema |4/4|
| native frame allocation failure outside a preserved I2/K1 diagnostic |5/5|
| admitted traversal exceeds8192 batches/16384 tokens |4/6|
| duplicate/aliased or misaligned destinations/staging controls/data |1/8|
| destination already borrowed/pinned at admission |2/8|
| private scratch/control/readiness invariant defect |5/9 before mutation; admitted cleanup defect terminates|

Frame-check/create/acquire precedence is registry identity, live phase, OWNER
identity/token, fixed profile, then storage metadata/alias/control checks. Stage
precedence is authenticated acquired frame/expected plane, readable view prefix,
dtype/device/layout/offset, rank/extents/span/alignment/alias, then element domains
in row order (all IDs/targets, both mask bytes, nonempty mask). Malformed view
prefix/null span is1/8; bad bool/nonempty domain is1/4. Readable-pointer trusted
preconditions remain unchanged. K1/I2 failures generated by actual upstream calls
retain their own domain/category/code and precedence, rather than this local map.
No local recoverable error after a publication or cleanup first write is permitted.
...
Every status-returning wrapper explicitly tests `(= status 0)`; Scheme integers,
including0, must never be used as success booleans. Expected native failures in
role/metric/staging wrappers snapshot the first
error and return#f; success returns#t. The literal Eshkol schedule short-circuits
at the first#f (fixed `and` sequence, not a C model loop), and the outer traversal
stops before any subsequent role/batch/finalize. Unexpected dependency exceptions
are caught by the already-installed handler and classified in its own region.
Neither an expected native rejection nor its D2 consumer constructs/raises an E1
error inside the obligation-bearing path; final delivery follows section7.
...
3. P1 enter writes eval to all17 nodes after preflight. Traverse D2 in Eshkol,
   ...
   CE/reduce/correct/accumulate. Release each exact batch with e3-d2-release-owned! before region close.
4. At EOS reject empty, else finalize. On failure snapshot the first mapped error
   ...
   its existing guard, then closed-release the exact owned batch; entry-live batches
...
The exact source fail-stop binding in e3_private_extension.esk is
`(extern void e3-native-invariant-fail :real abort)`, arity0, using already-linked
libc abort. It is source-private, not another C wrapper or public operation. Call
only for impossible admitted cleanup invariants; ordinary recoverable failures
still restore and deliver the mapped error. Void lowering is a compiled gate;
no invented runtime noreturn guarantee is needed.
...
Three independent Astra/high review lanes approve submission of this concrete
design. Numerical and composition/lifetime final reviews bind behavioral main
SHA256 `78576506dc3b97468671c66e15952a86ad10626868a6610c2e1a127d85af2ac8`
(before this evidence-only paragraph), BOOL companion
`0a8097886bc5530c97bd4d3edf2d9b0b2ea39ab87cd113f546f0e0c5512f5221`, and
D2 companion `9b46b6594601d03b36538468aaefc213429e1bd6039bee9898b1e552e0a816e0`.
Packaging approved the exact tuple/manifest section at earlier main
`0ab4e8fb45c25066e998fc2b72553e519a4edce4d2eb8fc351b4383e0c8439cc`, then
separately approved closed batch release and normal status-return consumers;
its explicit status-to-boolean clarification is applied. Those package rules
remain unchanged in the final behavioral version.

| Review | Concrete corrections incorporated |
|---|---|
| Numerical | Actual L3S N/W/mean order, persistent prior-counter readability, explicit M3T→E3 code translation, real begin/end scope proof, exact local rejection/status ordering |
| Composition/lifetime | All17 modes, root-stable D2 enrollment, source error index15, incomplete/drained abort phases, full setup/unwind order, pure native frame admission, guard-free release and libc fail-stop |
| Packaging | Exact driver root/export exception, always-test-only tuple, acyclic manifest hashes, all required builder branches/flags/include roots, retained D2 snapshot vector |

The original G3-T owner independently found no affected-contract conflict in E3's
normal failure-sentinel exit followed by raise outside the unchanged shared guard;
that was a bounded guard/error-impact finding, not whole-E3 approval. E3 and its
independent package reviewer approved the shared exact source/header before PR#102
merged. No G3 source or contract was modified by this proposal.

Checks run: `/usr/bin/bash scripts/check-ci-topology.sh` passes the merged
**19 shared suites/27-command coverage** inventory; `git diff --check` passes;
all7 local links in the five new documents resolve. A read-only comparison proves
only7 documentation files differ from merged main2ac0b5b and preserves the exact
upstream CI workflow/Makefile/topology/prerequisite scripts, shared contract,
G3-T contract and E3 architecture bytes. The earlier18/26 topology check was on
27c99f7 and is superseded by this merged-main structural check. No numerical suite,
production build, declaration probe, toolchain build or full CI was run by E3 in
this phase. The topology check is not execution of those19 suites.
