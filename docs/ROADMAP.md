...
| C2 | [Detached full training-state checkpoint schema](C2_TRAINING_STATE.md) | C1, D2, O2, X1, K2 | Accepted 1.0 schema/bytes; private strict same-fd parser/reader/inspect, model/O2 exact-bit staging, deep-owner lifecycle, policy and X1/D2 validation, atomic full SAVE, and K2-independent staged LOAD/reconstruction with exact rollback/resource proofs are gated; real K2 admission and public LOAD/capability, operational-limit evidence, exact K2-composed manifests, final independent review, and supported CI remain | active |
