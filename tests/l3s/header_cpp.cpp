...
static_assert(ET_L3S_MASKED_OBJECTIVE_ABI_MAJOR == 1u);
static_assert(ET_L3S_MASKED_OBJECTIVE_ABI_MINOR == 0u);
static_assert(std::is_same_v<decltype(&et_l3s_kernel_provider_v1),
                             const et_kernel_provider_v1 *(*)()>);

int main() {
  const et_kernel_provider_v1 *provider = et_l3s_kernel_provider_v1();
