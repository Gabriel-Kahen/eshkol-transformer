...
upstream in [PR #714](https://github.com/tsotchke/eshkol/pull/714). The isolated
dependency union at `d9887fb` / tree `e4bf84d` preserves that exact pin and the
reviewed #117/#121/TR3 leaves. A fresh network-isolated Ubuntu 22.04/LLVM 21.1.8
production-runtime rebuild and the full aggregate build pass, as do the final C2
allocation witness, C2 owner and LOAD gates, TR3-O transaction gate, and TR3-C D2
restore gate. Static integration review approved the exact union. The full registered
test union, legacy frozen-witness repinning, supported integration CI, and final
integration acceptance remain pending. No workstream status changes solely from the
pin update.
...
| E3 | [Loss, perplexity, token accuracy and validation runner](E3_EVALUATION_PROPOSAL.md) | M3/M3T, D2, L2/L3S; shared guard and bool metric seam accepted; E3 frame integration and P1/D2 restore pending | #99; the shared-call implementation, E3-METRICS and E3-D2 prerequisite union is accepted at merged `33a54ef` after supported 19-suite/29-command CI and focused merged-head checks. The exact nine-file deterministic reference/oracle toolkit from `37a8078` remains development-only. The exact five-file bounded native-frame leaf from `38fb989` is independently source-approved; root authenticated its supported normal and ASan/UBSan/LSan runs, each reporting 4,381 checks. The independently approved six-file parity leaf from `e086a61` has supported five-case normal/sanitizer/repeat evidence across all 21 roles and strict metrics. This candidate preserves both exact leaves and registers their standalone gates in the 19-suite/34-command union using the existing pinned development oracle. Integration review, full CI and root acceptance remain pending. Public evaluator/source composition, final runtime pin adoption, P1/D2 restoration, guard adoption and full evaluator acceptance remain pending | active |
...
| W3-GUARD | Checked-allocation cleanup ordering | M3 shared calls, C2 state borrows, runtime repair #108 | [M3 call guard #117](https://github.com/Gabriel-Kahen/eshkol-transformer/issues/117) has independently reviewed source plus passing ordinary and final-runtime handler-failure evidence. [C2 cleanup ordering #121](https://github.com/Gabriel-Kahen/eshkol-transformer/issues/121) covers eight audited C2 handler-setup windows plus the concrete P1 owned-release setup prerequisite; its final allocation witness and focused owner/LOAD gates pass on the exact runtime successor. The `d9887fb` isolated union adds the exact reserve dependency to all ten P1-containing aggregate manifests; its full aggregate build, TR3-O, and TR3-C D2 restore gates also pass under the supported toolchain. Full inherited package/retention gates, the registered test union, supported union CI, and integration acceptance remain pending; no new public API or checkpoint format | active |
