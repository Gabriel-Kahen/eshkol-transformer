...
evidence="${1:-$(project_build_dir)/g3t-native-context}"
mkdir -p "$evidence"
python3 "$PROJECT_ROOT/scripts/check-g3t-native-context.py" >"$evidence/static.stdout"
PYTHONDONTWRITEBYTECODE=1 python3 -m unittest -v tests.q0.test_python_isolation \
  >"$evidence/q0.stdout" 2>"$evidence/q0.stderr"
temporary="$(mktemp -d "${TMPDIR:-/tmp}/g3t-native-context.XXXXXX")"
trap 'status=$?; if (( status == 0 )); then rm -rf -- "$temporary"; else echo "G3-T native context build retained: $temporary" >&2; fi' EXIT
...
build_mode() {
  ...
  ar rcsD "$directory/libg3t_native_context.a" "$directory"/objects/*.o
...
WRAPPER
  ...
  env -u ESHKOL_PATH -u ESHKOL_JIT_CACHE_DIR ESHKOL_JIT_CACHE=0 \
    ...
    -L "$directory" --lib g3t_native_context \
    "$PROJECT_ROOT/tests/g3t/native_context_test.esk" -o "$directory/caller" \
  ...
  grep -E '^G3-T authentic native context PASS: checks=[1-9][0-9]*$' \
...
sha256sum "$PROJECT_ROOT/native/g3t_native_context_source_closure.txt" >"$evidence/closure.sha256"
cat "$evidence/static.stdout"
cat "$evidence/normal.stdout"
printf 'G3-T native context evidence: %s\n' "$evidence"
