...
evidence="${1:-$(project_build_dir)/g3c4-logits-reservation-test}"
mkdir -p "$evidence"
python3 "${PROJECT_ROOT}/scripts/check-g3c4-logits-reservation.py" >"$evidence/static.stdout"
...
if [[ "${G3C4_SKIP_LOGITS_PREDECESSOR:-0}" == 1 ]]; then
  printf 'G3-C4 output envelope regression: SKIPPED\n' >"$evidence/output-envelope-regression.stdout"
else
  CC="$cc" "${PROJECT_ROOT}/scripts/test-g3c4-output-envelope.sh" \
    "$evidence/output-envelope-regression" >"$evidence/output-envelope-regression.stdout" 2>&1
fi
...
"$cc" "${flags[@]}" -O2 "${base_macros[@]}" -c \
  "${PROJECT_ROOT}/src/eshkol_transformer/g3c4_model_owner.c" -o "$evidence/base-owner.o"
"$cc" "${flags[@]}" -O2 "${base_macros[@]}" \
  -DET_G3C4_LOGITS_RESERVATION_PRIVATE -c \
  "${PROJECT_ROOT}/src/eshkol_transformer/g3c4_model_owner.c" -o "$evidence/logits-owner.o"
for object in base-owner logits-owner; do
  nm -g --defined-only --format=posix "$evidence/$object.o" |
    awk 'NF >= 2 { print $1 }' | LC_ALL=C sort -u >"$evidence/$object-defined.txt"
done
comm -13 "$evidence/base-owner-defined.txt" "$evidence/logits-owner-defined.txt" \
  >"$evidence/added-defined.txt"
printf '%s\n' et_g3c4_private_logits_reserve_v1 |
  cmp - "$evidence/added-defined.txt"
if "$cc" "${flags[@]}" -O2 -DET_G3C4_LOGITS_RESERVATION_PRIVATE -c \
   "${PROJECT_ROOT}/src/eshkol_transformer/g3c4_model_owner.c" \
   -o "$evidence/invalid-logits-only.o" \
   >"$evidence/invalid-logits-only.stdout" 2>"$evidence/invalid-logits-only.stderr"; then
  printf 'invalid logits-only macro tuple compiled\n' >&2; exit 1
fi
grep -F 'ET_G3C4_LOGITS_RESERVATION_PRIVATE requires an active generator call and typed tensor release' \
  "$evidence/invalid-logits-only.stderr" >/dev/null
native_sources=(
  "${PROJECT_ROOT}/tests/g3c4/test_logits_reservation.c"
...
compile_mode() {
  ...
  "$cc" "${flags[@]}" "${mode_flags[@]}" \
    ...
    -o "$evidence/$mode-logits-reservation"
  "${runtime[@]}" timeout --foreground --signal=TERM --kill-after=5s 120s \
    "$evidence/$mode-logits-reservation" \
    >"$evidence/$mode.stdout" 2>"$evidence/$mode.stderr"
  test ! -s "$evidence/$mode.stderr"
  grep -E '^G3-C4 manual logits reservation PASS: checks=[1-9][0-9]* routes=2 f32-cuts=4 owner-cuts=1 borrow-cuts=2$' \
    "$evidence/$mode.stdout" >/dev/null
}
compile_mode normal
"$evidence/normal-logits-reservation" >"$evidence/runtime-repeat.stdout" \
...
sha256sum "${PROJECT_ROOT}/native/g3c4_logits_reservation_source_closure.txt" \
  >"$evidence/closure.sha256"
cat "$evidence/static.stdout"
cat "$evidence/normal.stdout"
printf 'G3-C4 private manual logits reservation evidence: %s\n' "$evidence"
