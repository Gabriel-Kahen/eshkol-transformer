...
evidence="${1:-$(project_build_dir)/g3c4-manual-at-test}"
mkdir -p "$evidence"
python3 "${PROJECT_ROOT}/scripts/check-g3c4-manual-at.py" >"$evidence/static.stdout"
for manifest in \
...
if [[ "${G3C4_SKIP_AT_PREDECESSOR:-0}" == 1 ]]; then
  printf 'G3-C4 A2 regression: SKIPPED\n' >"$evidence/a2-regression.stdout"
else
  CC="$cc" "${PROJECT_ROOT}/scripts/test-g3c4-manual-a2.sh" \
    "$evidence/a2-regression" >"$evidence/a2-regression.stdout" 2>&1
fi
...
"$cc" "${flags[@]}" -O2 "${base_macros[@]}" \
  ...
  -DET_G3C4_MANUAL_ROLE0_PRIVATE -DET_G3C4_MANUAL_PRE_A2_PRIVATE -DET_G3C4_MANUAL_A2_PRIVATE -c \
  "${PROJECT_ROOT}/src/eshkol_transformer/g3c4_model_owner.c" -o "$evidence/a2-owner.o"
"$cc" "${flags[@]}" -O2 "${base_macros[@]}" \
  -DET_G3C4_LOGITS_RESERVATION_PRIVATE -DET_G3C4_MANUAL_FRAME_BEGIN_PRIVATE \
  -DET_G3C4_MANUAL_ROLE0_PRIVATE -DET_G3C4_MANUAL_PRE_A2_PRIVATE -DET_G3C4_MANUAL_A2_PRIVATE -DET_G3C4_MANUAL_AT_PRIVATE -c \
  "${PROJECT_ROOT}/src/eshkol_transformer/g3c4_model_owner.c" -o "$evidence/manual-at-owner.o"
for object in a2-owner manual-at-owner; do
  nm -g --defined-only --format=posix "$evidence/$object.o" |
    awk 'NF >= 2 { print $1 }' | LC_ALL=C sort -u >"$evidence/$object-defined.txt"
done
comm -13 "$evidence/a2-owner-defined.txt" "$evidence/manual-at-owner-defined.txt" \
  >"$evidence/added-defined.txt"
test ! -s "$evidence/added-defined.txt"
if "$cc" "${flags[@]}" -O2 -DET_G3C4_MANUAL_AT_PRIVATE -c \
   "${PROJECT_ROOT}/src/eshkol_transformer/g3c4_model_owner.c" \
   -o "$evidence/invalid-at-only.o" \
   >"$evidence/invalid-at-only.stdout" 2>"$evidence/invalid-at-only.stderr"; then
  printf 'invalid at-only macro tuple compiled\n' >&2; exit 1
fi
grep -F 'ET_G3C4_MANUAL_AT_PRIVATE requires manual A2 attention' \
  "$evidence/invalid-at-only.stderr" >/dev/null
native_sources=(
  "${PROJECT_ROOT}/tests/g3c4/test_manual_at.c"
...
compile_mode() {
  ...
  "$cc" "${flags[@]}" "${mode_flags[@]}" \
    ...
    -o "$evidence/$mode-manual-at"
  "${runtime[@]}" timeout --foreground --signal=TERM --kill-after=5s 120s \
    "$evidence/$mode-manual-at" \
    >"$evidence/$mode.stdout" 2>"$evidence/$mode.stderr"
  test ! -s "$evidence/$mode.stderr"
  grep -E '^G3-C4 internal manual AT PASS: checks=[1-9][0-9]* routes=3 ordinal=11$' \
    "$evidence/$mode.stdout" >/dev/null
}
compile_mode normal
"$evidence/normal-manual-at" >"$evidence/runtime-repeat.stdout" \
...
sha256sum "${PROJECT_ROOT}/native/g3c4_manual_at_source_closure.txt" \
  >"$evidence/closure.sha256"
cat "$evidence/static.stdout"
cat "$evidence/normal.stdout"
printf 'G3-C4 private manual AT evidence: %s\n' "$evidence"
