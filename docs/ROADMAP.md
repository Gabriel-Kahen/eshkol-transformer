...
| C2 | [Detached full training-state checkpoint schema](C2_TRAINING_STATE.md) | C1, D2, O2, X1, K2 | K2 and all component prerequisites are merged. Private parser/reader/inspect, exact-bit staging, canonical deep-owner lifetime, staged LOAD/reconstruction, and atomic SAVE pass. The source-composed public candidate has the exact 81-global/75-export/81-string boundary, supplied-report K2 admission smoke coverage, authenticated policy/metadata carriers, fresh-cache lifecycle/isolation/determinism gates, flat 1,024/8,192 policy/metadata root retention, and poison/IR publication proof. Private stage/reconstruct development-host joint runs measured 517,280 and 519,428 KiB below the unchanged 524,288 KiB ceiling; the equivalent 64-tensor public K2 path, counted per-tensor admission/failure injection, supported Ubuntu 22.04/LLVM 21 CI, full predecessor reruns, and independent integration acceptance remain outstanding. Component continuation and atomic file publication only; TR3 retains joint live restore/full trajectory and G3 generation proof | active |
