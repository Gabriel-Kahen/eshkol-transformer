...
evidence="${1:-$(project_build_dir)/g3c4-manual-transport-test}"
mkdir -p "$evidence"
python3 "${PROJECT_ROOT}/scripts/check-g3c4-manual-transport.py" >"$evidence/static.stdout"
...
if [[ "${G3C4_SKIP_MANUAL_TRANSPORT_REGRESSIONS:-0}" == 1 ]]; then
  printf 'G3-C4 manual role step regression: SKIPPED\n' >"$evidence/manual-role-step-regression.stdout"
  printf 'G3-C4 output envelope regression: SKIPPED\n' >"$evidence/output-envelope-regression.stdout"
else
  CC="$cc" "${PROJECT_ROOT}/scripts/test-g3c4-manual-role-step.sh" \
    "$evidence/manual-role-step-regression" >"$evidence/manual-role-step-regression.stdout" 2>&1
...
tmp="$(mktemp -d "${TMPDIR:-/tmp}/eshkol-g3c4-manual-transport.XXXXXX")"
cleanup() {
  status=$?
  if (( status == 0 )); then rm -rf -- "$tmp"; else
    printf 'G3-C4 manual transport failure evidence retained at %s\n' "$tmp" >&2
...
compile_mode() {
  ...
  "$cc" "${common[@]}" "${mode_flags[@]}" -DET_A2_KV_CACHE_TESTING \
    -c "${PROJECT_ROOT}/tests/g3c4/manual_transport_native.c" \
    -o "$directory/objects/manual_transport_native.o"
  ...
  ar rcsD "$directory/libg3c4_manual_transport.a" "$directory"/objects/*.o
  sanitize_link=
...
WRAPPER
  ...
  for test in manual_transport_test; do
    env -u ESHKOL_PATH -u ESHKOL_JIT_CACHE_DIR ESHKOL_JIT_CACHE=0 \
      ...
      -L "$directory" --lib g3c4_manual_transport \
  ...
  grep -E '^G3-C4 manual transport PASS: checks=[1-9][0-9]* routes=3 rollback=3$' \
    "$directory/manual_transport_test.stdout" >/dev/null
}
compile_mode normal
for test in manual_transport_test; do
...
sha256sum "${PROJECT_ROOT}/native/g3c4_manual_transport_source_closure.txt" >"$evidence/closure.sha256"
sha256sum "$evidence/static.stdout" \
  "$evidence/normal-manual_transport_test.stdout" \
  "$evidence/manual_transport_test-runtime-repeat.stdout" \
  "$evidence/sanitize-manual_transport_test.stdout" \
  >"$evidence/result-seal.sha256"
cat "$evidence/static.stdout"
cat "$evidence/normal-manual_transport_test.stdout"
printf 'G3-C4 private manual transport evidence: %s\n' "$evidence"
