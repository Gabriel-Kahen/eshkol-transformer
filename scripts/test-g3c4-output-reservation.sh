...
evidence="${1:-${PROJECT_ROOT}/build/g3c4-output-reservation-test}"
cc="${CC:-clang}"
mkdir -p "${evidence}"
...
python3 "${PROJECT_ROOT}/scripts/check-g3c4-output-reservation.py" \
...
output_macros=("${base_macros[@]}" -DET_G3C4_OUTPUT_RESERVATION_PRIVATE)

"${cc}" "${flags[@]}" -O2 "${base_macros[@]}" -c \
  "${PROJECT_ROOT}/src/eshkol_transformer/g3c4_model_owner.c" \
  -o "${evidence}/base-owner.o"
"${cc}" "${flags[@]}" -O2 "${output_macros[@]}" -MMD \
  -MF "${evidence}/output.d" -c \
  "${PROJECT_ROOT}/src/eshkol_transformer/g3c4_model_owner.c" \
  -o "${evidence}/output-owner.o"
for object in base-owner output-owner; do
...
comm -13 "${evidence}/base-owner-defined.txt" \
  "${evidence}/output-owner-defined.txt" \
  >"${evidence}/added-defined.txt"
printf '%s\n' \
  et_g3c4_private_output_release_v1 \
  et_g3c4_private_output_reserve_v1 |
...
if "${cc}" "${flags[@]}" -O2 -DET_G3C4_OUTPUT_RESERVATION_PRIVATE -c \
    "${PROJECT_ROOT}/src/eshkol_transformer/g3c4_model_owner.c" \
    -o "${evidence}/invalid-output-only.o" \
    >"${evidence}/invalid-output-only.stdout" \
    2>"${evidence}/invalid-output-only.stderr"; then
  printf 'invalid output-only macro tuple compiled\n' >&2
  exit 1
fi
grep -F 'ET_G3C4_OUTPUT_RESERVATION_PRIVATE requires Step 18A' \
  "${evidence}/invalid-output-only.stderr" >/dev/null

sources=(
  "${PROJECT_ROOT}/tests/g3c4/test_output_reservation.c"
...
compile_mode() {
  ...
  "${cc}" "${flags[@]}" "${mode_flags[@]}" \
    ...
    -o "${evidence}/output-reservation-${mode}"
  "${environment[@]}" "${evidence}/output-reservation-${mode}" \
    >"${evidence}/${mode}.stdout" 2>"${evidence}/${mode}.stderr"
  test ! -s "${evidence}/${mode}.stderr"
}

compile_mode normal
"${evidence}/output-reservation-normal" \
...
sha256sum "${PROJECT_ROOT}/native/g3c4_output_reservation_source_closure.txt" \
  >"${evidence}/closure.sha256"
cat "${evidence}/static.stdout"
cat "${evidence}/normal.stdout"
printf 'G3-C4 private output reservation evidence: %s\n' "${evidence}"
