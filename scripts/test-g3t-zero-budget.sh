...
evidence="${1:-$(project_build_dir)/g3t-zero-budget}"
mkdir -p "$evidence"
...
trap 'status=$?; if (( status == 0 )); then rm -rf -- "$temporary"; else echo "G3-T zero budget build retained: $temporary" >&2; fi' EXIT
...
build_mode() {
  ...
  ar rcsD "$directory/libg3t_zero_budget.a" "$directory"/objects/*.o
...
WRAPPER
  ...
  env -u ESHKOL_PATH -u ESHKOL_JIT_CACHE_DIR ESHKOL_JIT_CACHE=0 \
    ...
    -L "$directory" --lib g3t_zero_budget \
    "$PROJECT_ROOT/tests/g3t/zero_budget_test.esk" -o "$directory/caller" \
  ...
  grep -E '^G3-T zero budget PASS: checks=[1-9][0-9]*$' \
...
sha256sum "$PROJECT_ROOT/native/g3t_zero_budget_source_closure.txt" >"$evidence/closure.sha256"
cat "$evidence/static.stdout"
cat "$evidence/normal.stdout"
printf 'G3-T zero budget evidence: %s\n' "$evidence"
