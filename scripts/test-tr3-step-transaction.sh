...
[[ -z "$(git status --porcelain --untracked-files=all)" ]] || \
  die "step transaction gate requires a clean committed checkout"
evidence="${TR3_STEP_TX_EVIDENCE_DIR:-$(mktemp -d "${TMPDIR:-/tmp}/tr3-step-tx.XXXXXX")}"
mkdir -p -- "${evidence}"
...
/usr/bin/time -v -o /out/compile.time \
  ...
  /candidate/eshkol-build-canonical/eshkol-run --strict-types --no-stdlib -O 0 \
    ...
    /workspace/tests/tr3_step_transaction/runtime.esk -o /out/runtime \
...
grep -Fx 'TR3-STEP-TX-PASS checks=63' "${evidence}/runtime.stdout" >/dev/null
...
def visit(path):
    ...
    for name in re.findall(r'\(load\s+"([^"]+)"\)', path.read_text()):
        ...
        visit(found)
visit(root / 'tests/tr3_step_transaction/runtime.esk')
paths = sorted(str(path.relative_to(root)) for path in seen)
for required in ('native/tr3_step_transaction_root.esk',
...
printf 'TR3 step transaction evidence: %s\n' "${evidence}"
printf 'TR3 step transaction seal: %s\n' \
