...
evidence="${1:-$(project_build_dir)/g3c4-manual-tail-test}"
mkdir -p "$evidence"
python3 "${PROJECT_ROOT}/scripts/check-g3c4-manual-tail.py" >"$evidence/static.stdout"
for manifest in \
...
if [[ "${G3C4_SKIP_TAIL_PREDECESSOR:-0}" == 1 ]]; then
  printf 'G3-C4 R regression: SKIPPED\n' >"$evidence/r-regression.stdout"
else
  CC="$cc" "${PROJECT_ROOT}/scripts/test-g3c4-manual-r.sh" \
    "$evidence/r-regression" >"$evidence/r-regression.stdout" 2>&1
fi
...
"$cc" "${flags[@]}" -O2 "${base_macros[@]}" \
  ...
  -DET_G3C4_MANUAL_ROLE0_PRIVATE -DET_G3C4_MANUAL_PRE_A2_PRIVATE -DET_G3C4_MANUAL_A2_PRIVATE -DET_G3C4_MANUAL_AT_PRIVATE -DET_G3C4_MANUAL_AO_PRIVATE -DET_G3C4_MANUAL_R_PRIVATE -c \
  "${PROJECT_ROOT}/src/eshkol_transformer/g3c4_model_owner.c" -o "$evidence/r-owner.o"
"$cc" "${flags[@]}" -O2 "${base_macros[@]}" \
  -DET_G3C4_LOGITS_RESERVATION_PRIVATE -DET_G3C4_MANUAL_FRAME_BEGIN_PRIVATE \
  ...
  "${PROJECT_ROOT}/src/eshkol_transformer/g3c4_model_owner.c" -o "$evidence/manual-tail-owner.o"
for object in r-owner manual-tail-owner; do
  nm -g --defined-only --format=posix "$evidence/$object.o" |
    awk 'NF >= 2 { print $1 }' | LC_ALL=C sort -u >"$evidence/$object-defined.txt"
done
comm -13 "$evidence/r-owner-defined.txt" "$evidence/manual-tail-owner-defined.txt" \
  >"$evidence/added-defined.txt"
test ! -s "$evidence/added-defined.txt"
if "$cc" "${flags[@]}" -O2 -DET_G3C4_MANUAL_TAIL_PRIVATE -c \
   "${PROJECT_ROOT}/src/eshkol_transformer/g3c4_model_owner.c" \
   -o "$evidence/invalid-tail-only.o" \
   >"$evidence/invalid-tail-only.stdout" 2>"$evidence/invalid-tail-only.stderr"; then
  printf 'invalid tail-only macro tuple compiled\n' >&2; exit 1
fi
grep -F 'ET_G3C4_MANUAL_TAIL_PRIVATE requires manual attention residual' \
  "$evidence/invalid-tail-only.stderr" >/dev/null
native_sources=(
  "${PROJECT_ROOT}/tests/g3c4/test_manual_tail.c"
...
compile_mode() {
  ...
  "$cc" "${flags[@]}" "${mode_flags[@]}" \
    ...
    -o "$evidence/$mode-manual-tail"
  "${runtime[@]}" timeout --foreground --signal=TERM --kill-after=5s 120s \
    "$evidence/$mode-manual-tail" \
    >"$evidence/$mode.stdout" 2>"$evidence/$mode.stderr"
  test ! -s "$evidence/$mode.stderr"
  grep -E '^G3-C4 internal manual tail PASS: checks=[1-9][0-9]* routes=3 cuts=21 ordinals=14-20$' \
    "$evidence/$mode.stdout" >/dev/null
}
compile_mode normal
"$evidence/normal-manual-tail" >"$evidence/runtime-repeat.stdout" \
...
sha256sum "${PROJECT_ROOT}/native/g3c4_manual_tail_source_closure.txt" \
  >"$evidence/closure.sha256"
cat "$evidence/static.stdout"
cat "$evidence/normal.stdout"
printf 'G3-C4 private manual tail evidence: %s\n' "$evidence"
