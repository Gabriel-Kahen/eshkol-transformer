...
evidence="${1:-$(project_build_dir)/g3t-output-text}"
mkdir -p "$evidence"
...
temporary="$(mktemp -d "${TMPDIR:-/tmp}/g3t-output-text.XXXXXX")"
trap 'status=$?; if (( status == 0 )); then rm -rf -- "$temporary"; else echo "G3-T output text build retained: $temporary" >&2; fi' EXIT
...
build_mode() {
  ...
  ar rcsD "$directory/libg3t_output_text.a" "$directory"/objects/*.o
...
WRAPPER
  ...
  env -u ESHKOL_PATH -u ESHKOL_JIT_CACHE_DIR ESHKOL_JIT_CACHE=0 \
    ...
    -L "$directory" --lib g3t_output_text \
    "$PROJECT_ROOT/tests/g3t/output_text_test.esk" -o "$directory/caller" \
  ...
  grep -E '^G3-T output text PASS: checks=[1-9][0-9]*$' \
...
sha256sum "$PROJECT_ROOT/native/g3t_output_text_source_closure.txt" >"$evidence/closure.sha256"
cat "$evidence/static.stdout"
cat "$evidence/normal.stdout"
printf 'G3-T output text evidence: %s\n' "$evidence"
