...
evidence="${1:-$(project_build_dir)/g3c4-manual-frame-begin-test}"
mkdir -p "$evidence"
python3 "${PROJECT_ROOT}/scripts/check-g3c4-manual-frame-begin.py" >"$evidence/static.stdout"
...
if [[ "${G3C4_SKIP_FRAME_PREDECESSOR:-0}" == 1 ]]; then
  printf 'G3-C4 logits reservation regression: SKIPPED\n' >"$evidence/logits-reservation-regression.stdout"
else
  CC="$cc" "${PROJECT_ROOT}/scripts/test-g3c4-logits-reservation.sh" \
    "$evidence/logits-reservation-regression" >"$evidence/logits-reservation-regression.stdout" 2>&1
fi
...
"$cc" "${flags[@]}" -O2 "${base_macros[@]}" \
  -DET_G3C4_LOGITS_RESERVATION_PRIVATE -DET_G3C4_MANUAL_FRAME_BEGIN_PRIVATE -c \
...
printf '%s\n' et_g3c4_private_frame_begin_v1 |
  cmp - "$evidence/added-defined.txt"
if "$cc" "${flags[@]}" -O2 -DET_G3C4_MANUAL_FRAME_BEGIN_PRIVATE -c \
   "${PROJECT_ROOT}/src/eshkol_transformer/g3c4_model_owner.c" \
   -o "$evidence/invalid-frame-only.o" \
   >"$evidence/invalid-frame-only.stdout" 2>"$evidence/invalid-frame-only.stderr"; then
  printf 'invalid frame-only macro tuple compiled\n' >&2; exit 1
fi
grep -F 'ET_G3C4_MANUAL_FRAME_BEGIN_PRIVATE requires logits, input and committed-cache prerequisites' \
  "$evidence/invalid-frame-only.stderr" >/dev/null
native_sources=(
  "${PROJECT_ROOT}/tests/g3c4/test_manual_frame_begin.c"
...
compile_mode() {
  ...
  "$cc" "${flags[@]}" "${mode_flags[@]}" \
    ...
    -o "$evidence/$mode-manual-frame-begin"
  "${runtime[@]}" timeout --foreground --signal=TERM --kill-after=5s 120s \
    "$evidence/$mode-manual-frame-begin" \
    >"$evidence/$mode.stdout" 2>"$evidence/$mode.stderr"
  test ! -s "$evidence/$mode.stderr"
  grep -E '^G3-C4 manual frame begin PASS: checks=[1-9][0-9]* prefill=2 decode=1 allocation=1 borrow=2$' \
    "$evidence/$mode.stdout" >/dev/null
}
compile_mode normal
"$evidence/normal-manual-frame-begin" >"$evidence/runtime-repeat.stdout" \
...
sha256sum "${PROJECT_ROOT}/native/g3c4_manual_frame_begin_source_closure.txt" \
  >"$evidence/closure.sha256"
cat "$evidence/static.stdout"
cat "$evidence/normal.stdout"
printf 'G3-C4 private manual frame begin evidence: %s\n' "$evidence"
