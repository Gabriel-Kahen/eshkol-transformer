...
evidence="${1:-$(project_build_dir)/g3c4-manual-pre-a2-test}"
mkdir -p "$evidence"
python3 "${PROJECT_ROOT}/scripts/check-g3c4-manual-pre-a2.py" >"$evidence/static.stdout"
...
if [[ "${G3C4_SKIP_PRE_A2_PREDECESSOR:-0}" == 1 ]]; then
  printf 'G3-C4 role0 regression: SKIPPED\n' >"$evidence/role0-regression.stdout"
else
  CC="$cc" "${PROJECT_ROOT}/scripts/test-g3c4-manual-role0.sh" \
    "$evidence/role0-regression" >"$evidence/role0-regression.stdout" 2>&1
fi
...
"$cc" "${flags[@]}" -O2 "${base_macros[@]}" \
  -DET_G3C4_LOGITS_RESERVATION_PRIVATE -DET_G3C4_MANUAL_FRAME_BEGIN_PRIVATE \
  -DET_G3C4_MANUAL_ROLE0_PRIVATE -DET_G3C4_MANUAL_PRE_A2_PRIVATE -c \
  "${PROJECT_ROOT}/src/eshkol_transformer/g3c4_model_owner.c" -o "$evidence/pre-a2-owner.o"
for object in base-owner pre-a2-owner; do
  nm -g --defined-only --format=posix "$evidence/$object.o" |
    awk 'NF >= 2 { print $1 }' | LC_ALL=C sort -u >"$evidence/$object-defined.txt"
done
comm -13 "$evidence/base-owner-defined.txt" "$evidence/pre-a2-owner-defined.txt" \
  >"$evidence/added-defined.txt"
test ! -s "$evidence/added-defined.txt"
if "$cc" "${flags[@]}" -O2 -DET_G3C4_MANUAL_PRE_A2_PRIVATE -c \
   "${PROJECT_ROOT}/src/eshkol_transformer/g3c4_model_owner.c" \
   -o "$evidence/invalid-pre-a2-only.o" \
   >"$evidence/invalid-pre-a2-only.stdout" 2>"$evidence/invalid-pre-a2-only.stderr"; then
  printf 'invalid pre-a2-only macro tuple compiled\n' >&2; exit 1
fi
grep -F 'ET_G3C4_MANUAL_PRE_A2_PRIVATE requires the first manual role' \
  "$evidence/invalid-pre-a2-only.stderr" >/dev/null
native_sources=(
  "${PROJECT_ROOT}/tests/g3c4/test_manual_pre_a2.c"
...
compile_mode() {
  ...
  "$cc" "${flags[@]}" "${mode_flags[@]}" \
    ...
    -o "$evidence/$mode-manual-pre-a2"
  "${runtime[@]}" timeout --foreground --signal=TERM --kill-after=5s 120s \
    "$evidence/$mode-manual-pre-a2" \
    >"$evidence/$mode.stdout" 2>"$evidence/$mode.stderr"
  test ! -s "$evidence/$mode.stderr"
  grep -E '^G3-C4 internal manual pre-A2 PASS: checks=[1-9][0-9]* routes=3 cuts=27 ordinals=1-9$' \
    "$evidence/$mode.stdout" >/dev/null
}
compile_mode normal
"$evidence/normal-manual-pre-a2" >"$evidence/runtime-repeat.stdout" \
...
sha256sum "${PROJECT_ROOT}/native/g3c4_manual_pre_a2_source_closure.txt" \
  >"$evidence/closure.sha256"
cat "$evidence/static.stdout"
cat "$evidence/normal.stdout"
printf 'G3-C4 private manual pre-A2 evidence: %s\n' "$evidence"
