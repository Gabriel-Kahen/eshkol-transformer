...
evidence="${1:-$(project_build_dir)/g3t-final-publication}"
mkdir -p "$evidence"
...
temporary="$(mktemp -d "${TMPDIR:-/tmp}/g3t-final-publication.XXXXXX")"
trap 'status=$?; if (( status == 0 )); then rm -rf -- "$temporary"; else echo "G3-T final publication build retained: $temporary" >&2; fi' EXIT
...
build_mode() {
  ...
  ar rcsD "$directory/libg3t_final_publication.a" "$directory"/objects/*.o
...
WRAPPER
  ...
  env -u ESHKOL_PATH -u ESHKOL_JIT_CACHE_DIR ESHKOL_JIT_CACHE=0 \
    ...
    -L "$directory" --lib g3t_final_publication \
    "$PROJECT_ROOT/tests/g3t/final_publication_test.esk" -o "$directory/caller" \
  ...
  grep -E '^G3-T final publication PASS: checks=[1-9][0-9]*$' \
    "$directory/stdout" >/dev/null
}
build_mode normal
if [[ "${G3T_ONLY_NORMAL:-0}" == 1 ]]; then
...
sha256sum "$PROJECT_ROOT/native/g3t_final_publication_source_closure.txt" >"$evidence/closure.sha256"
cat "$evidence/static.stdout"
cat "$evidence/normal.stdout"
printf 'G3-T final publication evidence: %s\n' "$evidence"
