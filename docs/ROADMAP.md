...
| TR3 | Trainer state machine and exact resume | M3, L2, O2, D2, C2, E3, SHARED-R2 | #116 [private objective bridge](TR3B_OBJECTIVE_BRIDGE.md) is preserved from exact source `071abb72` with independent approval and a passing supported Ubuntu 22.04/LLVM 21.1.8 focused gate. [#119 private O2 update-and-clear](TR3_O_OPTIMIZER_TRANSACTION_PROPOSAL.md) is preserved from exact implementation source `783dc576` with local atomicity, numerical, lifetime and CI-registration evidence; independent review approved the exact `2006cde` B/O integration. [TR3-C live restore contract](https://github.com/Gabriel-Kahen/eshkol-transformer/issues/120) is accepted for implementation; its live transaction is not integrated on main. The bounded TR3-B/O prerequisite union is accepted through PR #124 at `f74fede` after independent review, supported 19-suite/34-command CI and exact-tree merged-main evidence reuse. Public trainer, joint restore, one-batch overfit, held-out improvement and interrupted/resumed equivalence remain pending | active |
| CLI3 | Corpus, tokenizer, pretrain, evaluate, generate and inspect CLIs | T2, TR3, G3, X1 | Six CLI3-A tokenizer/corpus/checkpoint-inspect commands passed focused supported-runtime review on isolated candidate `16a1303`; integration into main remains pending. Pretrain/evaluate/generate await their actual upstream APIs and end-to-end evidence | active |
