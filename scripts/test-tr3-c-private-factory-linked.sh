...
evidence="${TR3_FACTORY_LINKED_EVIDENCE_DIR:-$(mktemp -d "${TMPDIR:-/tmp}/tr3-factory-linked.XXXXXX")}"
mkdir -p -- "${evidence}"
...
clang++-21 -std=c++17 -O2 -Wall -Wextra -Werror -Wpedantic \
  ...
  -fno-exceptions -fno-rtti \
  -isystem /fixed-source/inc -isystem /fixed-source/lib/core \
  -I native -MMD -MF /out/native/tr3_c_private_factory_bridge.o.d \
  -c native/tr3_c_private_factory_bridge.cpp \
  -o /out/native/tr3_c_private_factory_bridge.o
...
awk "\$1 != \"et_tr3_c_private_initialize_v1\" && \
     \$1 != \"et_tr3_c_private_trainer_create_v1\" && \
     \$1 != \"et_tr3_c_private_trainer_close_v1\" {print}" /out/raw-defined.txt \
...
clang++-21 -std=c++17 -O0 -Wall -Wextra -Werror -Wpedantic \
  ...
  tests/tr3_private_factory/bridge_fixture.cpp \
  -o /out/factory-bridge-fixture
timeout 10s /out/factory-bridge-fixture \
  > /out/factory-bridge-success.stdout \
  2> /out/factory-bridge-success.stderr
...
clang-21 -std=c11 -Wall -Wextra -Werror -Wpedantic \
  -I native tests/tr3_private_factory/dynamic_probe.c \
...
PY
cmp native/tr3_c_private_factory_source_closure.txt "${evidence}/source-closure.txt"
...
(evidence / 'native-objects.txt').write_text(
    ''.join(path.name[:-2] + '\n' for path in deps))
PY
cmp native/tr3_c_private_factory_native_source_closure.txt \
  "${evidence}/native-source-closure.txt"
cmp native/tr3_c_private_factory_native_objects.txt \
...
grep -Fx 'TR3 factory fault lease-busy first=7/9/0/0 again=7/1/3/0 D2=1/1/0/0 O2=1 PASS' \
  "${evidence}/fault/lease-busy.stdout" >/dev/null
grep -Fx 'TR3 factory fault close-busy busy=7/10/2/0 retry=0/0/0/0 repeat=7/10/5/0 D2=1/0/1 PASS' \
  "${evidence}/fault/close-busy.stdout" >/dev/null
...
test ! -s "${evidence}/factory-bridge-success.stdout"
...
printf 'TR3 private factory linked candidate evidence: %s\n' "${evidence}"
printf 'TR3 private factory linked candidate seal: %s\n' \
