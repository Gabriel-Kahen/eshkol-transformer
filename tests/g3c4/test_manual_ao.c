#define ET_G3C4_MANUAL_AO_PRIVATE 1
#include "test_manual_ao_predecessor.inc"

static int record_ao, fail_ao;
static size_t ao_dispatches;
static et_g3c4_context_internal *ao_context;
static float ao_output[8];
...
int32_t __wrap_et_kernel_runtime_dispatch(
  ...
  int32_t status;
  if (record_ao) {
    const et_g3c4_manual_frame_internal *frame = ao_context->manual_frame;
    ...
    ao_dispatches++;
    CHECK(ao_dispatches == 1u && frame->next_ordinal == 12);
    CHECK(strcmp(call->capability,
                 t2 ? "n3k.linear" : "g3n.linear-forward") == 0);
    CHECK(strcmp(call->request->operation,
                 t2 ? "n3k.linear.forward-no-bias" :
                      "g3n.linear.forward-no-bias") == 0);
    CHECK(call->request->deterministic == 1u && call->request->rank == 4u &&
          ...
          call->request->shape[2] == 4u && call->request->shape[3] == 4u);
    CHECK(call->input_count == 2u && call->output_count == 1u &&
          inputs[0].data == frame->at && inputs[0].rank == 3u &&
          inputs[0].shape[0] == 1u &&
          inputs[0].shape[1] == (uint64_t)frame->input_length &&
          inputs[0].shape[2] == 4u &&
          inputs[0].byte_length == (size_t)frame->input_length * 4u * sizeof(float));
    CHECK(inputs[1].data == ao_context->pins.views[1].data &&
          inputs[1].rank == 2u && inputs[1].shape[0] == 4u &&
          inputs[1].shape[1] == 4u &&
          inputs[1].byte_length == 16u * sizeof(float));
    CHECK(outputs[0].data != frame->ao && outputs[0].rank == 3u &&
    ...
    if (fail_ao) {
      ...
      strcpy(error->operation, "g3c4.manual-ao-cut");
      strcpy(error->message, "injected attention-out failure");
      return ET_KERNEL_ERROR_INTERNAL;
    }
  }
  status = et_g3c4_manual_ao_base_dispatch(runtime, call, error);
  if (record_ao && status == 0)
    memcpy(ao_output, ((const et_kernel_tensor_view_v1 *)call->outputs)[0].data,
           (size_t)ao_context->manual_frame->input_length * 4u * sizeof(float));
  return status;
}

static void ao_case(et_g3c4_model_owner_internal *owner,
                    int64_t frame_kind, int64_t length) {
  const int64_t ids[2] = {41, 43}, prefix[1] = {47};
  float logits[256], at_before[8], ah_before[8];
  ...
  int64_t rng[4];
  CHECK(et_g3c4_manual_ao_run(context) == ET_G3C4_INVALID_STATE);
  if (frame_kind == 2) {
  ...
  CHECK(frame->next_ordinal == 12 && frame->a2_candidate != NULL &&
        frame->a2_transaction != NULL);
  candidate = frame->a2_candidate;
  transaction = frame->a2_transaction;
  memcpy(at_before, frame->at, sizeof(at_before));
  memcpy(ah_before, frame->ah, sizeof(ah_before));
  ...
  ao_dispatches = 0u;
  record_ao = fail_ao = 1;
  CHECK(et_g3c4_manual_ao_run(context) != 0);
  record_ao = fail_ao = 0;
  CHECK(ao_dispatches == 1u && frame->next_ordinal == 12 &&
  ...
  for (size_t i = 0u; i < 8u; i++) CHECK(frame->ao[i] == 0.0f);
  CHECK(memcmp(at_before, frame->at, sizeof(at_before)) == 0 &&
        memcmp(ah_before, frame->ah, sizeof(ah_before)) == 0);
  ...
  record_ao = 0;
  CHECK(ao_dispatches == 1u && frame->next_ordinal == 13 &&
        frame->a2_candidate == candidate && frame->a2_transaction == transaction);
  CHECK(memcmp(frame->ao, ao_output, (size_t)length * 4u * sizeof(float)) == 0);
  for (size_t i = 0u; i < (size_t)length * 4u; i++)
    CHECK(isfinite(frame->ao[i]));
  if (length == 1)
    for (size_t i = 4u; i < 8u; i++) CHECK(frame->ao[i] == 0.0f);
  CHECK(et_g3c4_manual_ao_run(context) == ET_G3C4_INVALID_STATE);
  CHECK(memcmp(at_before, frame->at, sizeof(at_before)) == 0 &&
        memcmp(ah_before, frame->ah, sizeof(ah_before)) == 0);
  snapshot_at_candidate(frame, &staged_after);
...
int main(void) {
  ...
  ao_case(owner, 1, 2);
  ao_case(owner, 2, 1);
  printf("G3-C4 internal manual AO PASS: checks=%zu routes=3 ordinal=12\n", checks);
  return 0;
}
