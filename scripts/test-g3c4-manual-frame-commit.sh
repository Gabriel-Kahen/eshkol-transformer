...
evidence="${1:-$(project_build_dir)/g3c4-manual-frame-commit-test}"
mkdir -p "$evidence"
python3 "${PROJECT_ROOT}/scripts/check-g3c4-manual-frame-commit.py" >"$evidence/static.stdout"
for manifest in \
...
if [[ "${G3C4_SKIP_FRAME_COMMIT_PREDECESSOR:-0}" == 1 ]]; then
  printf 'G3-C4 manual tail regression: SKIPPED\n' >"$evidence/tail-regression.stdout"
else
  CC="$cc" "${PROJECT_ROOT}/scripts/test-g3c4-manual-tail.sh" \
    "$evidence/tail-regression" >"$evidence/tail-regression.stdout" 2>&1
fi
...
"$cc" "${flags[@]}" -O2 "${base_macros[@]}" \
  ...
  -DET_G3C4_MANUAL_ROLE0_PRIVATE -DET_G3C4_MANUAL_PRE_A2_PRIVATE -DET_G3C4_MANUAL_A2_PRIVATE -DET_G3C4_MANUAL_AT_PRIVATE -DET_G3C4_MANUAL_AO_PRIVATE -DET_G3C4_MANUAL_R_PRIVATE -DET_G3C4_MANUAL_TAIL_PRIVATE -c \
  "${PROJECT_ROOT}/src/eshkol_transformer/g3c4_model_owner.c" -o "$evidence/tail-owner.o"
"$cc" "${flags[@]}" -O2 "${base_macros[@]}" \
  -DET_G3C4_LOGITS_RESERVATION_PRIVATE -DET_G3C4_MANUAL_FRAME_BEGIN_PRIVATE \
...
for object in tail-owner frame-commit-owner; do
  nm -g --defined-only --format=posix "$evidence/$object.o" |
    awk 'NF >= 2 { print $1 }' | LC_ALL=C sort -u >"$evidence/$object-defined.txt"
done
comm -13 "$evidence/tail-owner-defined.txt" "$evidence/frame-commit-owner-defined.txt" \
  >"$evidence/added-defined.txt"
grep -Fx 'et_g3c4_private_frame_prepare_v1' "$evidence/added-defined.txt" >/dev/null
grep -Fx 'et_g3c4_private_frame_commit_v1' "$evidence/added-defined.txt" >/dev/null
test "$(wc -l <"$evidence/added-defined.txt")" -eq 2
if "$cc" "${flags[@]}" -O2 -DET_G3C4_MANUAL_FRAME_COMMIT_PRIVATE -c \
   "${PROJECT_ROOT}/src/eshkol_transformer/g3c4_model_owner.c" \
   -o "$evidence/invalid-frame-commit-only.o" \
   >"$evidence/invalid-frame-commit-only.stdout" 2>"$evidence/invalid-frame-commit-only.stderr"; then
  printf 'invalid frame-commit-only macro tuple compiled\n' >&2; exit 1
fi
grep -F 'ET_G3C4_MANUAL_FRAME_COMMIT_PRIVATE requires the completed manual tail' \
  "$evidence/invalid-frame-commit-only.stderr" >/dev/null
native_sources=(
  "${PROJECT_ROOT}/tests/g3c4/test_manual_frame_commit.c"
...
compile_mode() {
  ...
  "$cc" "${flags[@]}" "${mode_flags[@]}" \
    ...
    -o "$evidence/$mode-manual-frame-commit"
  "${runtime[@]}" timeout --foreground --signal=TERM --kill-after=5s 120s \
    "$evidence/$mode-manual-frame-commit" \
    >"$evidence/$mode.stdout" 2>"$evidence/$mode.stderr"
  test ! -s "$evidence/$mode.stderr"
  grep -E '^G3-C4 private manual frame commit PASS: checks=[1-9][0-9]* routes=4$' \
    "$evidence/$mode.stdout" >/dev/null
}
compile_mode normal
"$evidence/normal-manual-frame-commit" >"$evidence/runtime-repeat.stdout" \
...
sha256sum "${PROJECT_ROOT}/native/g3c4_manual_frame_commit_source_closure.txt" \
  >"$evidence/closure.sha256"
cat "$evidence/static.stdout"
cat "$evidence/normal.stdout"
printf 'G3-C4 private manual frame commit evidence: %s\n' "$evidence"
