...
| G3-C4-NATIVE | [Seeded native owner and fixed-14 pin integration](g3/G3_C4_NATIVE_OWNER_PIN_INTEGRATION.md) | G3-C4-N, N3K, I2, M3/M3T | Integration-preparation candidate on reviewed runtime/reference base `d5fa9a7`: exact accepted owner production from `bd977aa`, exact accepted pin production from `e11e61f`, one inherited I2 registry, and conditional native-only surfaces. The pin test baseline is advanced to the integration base because its accepted rank-two I2 provider changes the compiled closure. The exact integrated native closure passes supported Clang 21 normal, repeat, ASan/UBSan/LSan, conditional-symbol, ordinary-object and C2 regression checks. Independent integration review and root acceptance remain pending. No active model seam, Eshkol/P1 construction, transport, cache, sampler, restore, package manifest or CI registration is added | review |
| TR3 | [Trainer state machine and exact resume](TR3_P1_FIXED_SET.md) | M3, L2, O2, D2, C2, E3, SHARED-R2 | #116 [private objective bridge candidate](TR3B_OBJECTIVE_BRIDGE.md) is preserved from exact source `071abb72` with independent approval and a passing supported Ubuntu 22.04/LLVM 21.1.8 focused gate. The bounded P1 slots69/70 fixed-set leaf preserves the accepted E3 prefix and public surface with exact zero-allocation/count/retention evidence and independent review. [#119 private O2 update-and-clear candidate](TR3_O_OPTIMIZER_TRANSACTION_PROPOSAL.md) is preserved from exact implementation source `783dc576` with local atomicity, numerical, lifetime and CI-registration evidence; independent review approved the exact `2006cde` B/O integration. The accepted [TR3-C live restore contract](TR3_C_LIVE_STATE_CONTRACT.md) now has bounded checked-I2 cleanup and feature-local private D2 restore leaf seams with focused sanitizer, symbol-isolation, strict-AOT, lifecycle, and independent-review evidence. The independently reviewed [development reference](TR3_REFERENCE_INTEGRATION.md) from `cc474900` is preserved exactly: 33 trajectory points, 14 parameters, 28 moments, weighted held-out and D2 cursor/EOS cases, with strict checksum/type validation; this supplies no native training or resume proof. Full supported union CI, lease registry, O2 overlap, trainer/compositor integration, public trainer, joint restore, one-batch overfit, held-out improvement and interrupted/resumed equivalence remain pending | active |
