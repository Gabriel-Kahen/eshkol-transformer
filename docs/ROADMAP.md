...
| TR3 | Trainer state machine and exact resume | M3, L2, O2, D2, C2, E3, SHARED-R2 | Contract design active; runtime unimplemented. Acceptance still requires one-batch overfit, held-out loss improvement and interrupted/resumed equivalence across complete state | active |
| CLI3 | Corpus, tokenizer, pretrain, evaluate, generate and inspect CLIs | T2, TR3, G3, X1 | Contract design active, including a dependency-ready data/inspect subset; commands unimplemented. End-to-end command tests and actionable diagnostics remain required | active |
