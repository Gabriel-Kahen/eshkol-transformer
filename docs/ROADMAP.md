...
| C2 | [Detached full training-state checkpoint schema](C2_TRAINING_STATE.md) | C1, D2, O2, X1, K2 | Prerequisites merged; private strict parser/reader/inspect, exact-bit staging, deep-owner lifecycle, staged LOAD/reconstruction and atomic SAVE are implemented, while lifetime/operational-bound repair, real K2 public composition, full gates and independent acceptance remain outstanding. Component continuation and atomic file publication only; TR3 retains joint live restore/full trajectory and G3 generation proof | active |
