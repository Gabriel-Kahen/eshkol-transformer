...
| E3-P1 | [Canonical fixed-model mode restoration](E3_P1_MODES.md) | Accepted E3/P1 contract; checked-promotion upstream prerequisite | #108; lexical64..68 draft preserves first64/public surface; real generated-store allocator-failure probe confirms [upstream713](https://github.com/tsotchke/eshkol/issues/713), separate diagnostic c388b42; local O0 ordinary749 checks pass; upstream caller/ABI plan refined and constructor prerequisite identified; root holds publication acceptance, full runtime/inheritor/resource evidence unaccepted | blocked |
...
| TR3 | [Trainer state machine and exact resume](TR3_P1_FIXED_SET.md) | M3, L2, O2, D2, C2, E3, SHARED-R2 | #116 [private objective bridge candidate](TR3B_OBJECTIVE_BRIDGE.md) is preserved from exact source `071abb72` with independent approval and a passing supported Ubuntu 22.04/LLVM 21.1.8 focused gate. The bounded P1 slots69/70 fixed-set leaf preserves the accepted E3 prefix and public surface with exact zero-allocation/count/retention evidence and independent review. [#119 private O2 update-and-clear candidate](TR3_O_OPTIMIZER_TRANSACTION_PROPOSAL.md) is preserved from exact implementation source `783dc576` with local atomicity, numerical, lifetime and CI-registration evidence; independent review approved the exact `2006cde` B/O integration. The accepted [TR3-C live restore contract](TR3_C_LIVE_STATE_CONTRACT.md) now has bounded checked-I2 cleanup and feature-local private D2 restore leaf seams with focused sanitizer, symbol-isolation, strict-AOT, lifecycle, and independent-review evidence. The bounded source-private lease-core candidate now supplies the sole logical registry/token, final-head-safe acquisition, installed O2 overlap closure, lease-aware component mutation wrappers, and fixed phase admissions. Its focused frozen-runtime gate passes authentic idle/overlap/reentry/head-substitution cases, public mutation preservation, and zero arena growth at 1,024/8,192 admission iterations; independent review found no blocking issue in this bounded leaf. A test-only failure successor preserves genuine bounded constructor, handler and checked-promotion prefix tests plus an allocation census from canonical staging through enrollment. Full allocation-prefix acceptance remains blocked. Final81298 generated code dereferences null results from raw arena, closure and string allocators before the lease guard can catch a condition. On frozen TR3 source `f602a66`, production-profile de0b249 passed the first 950 full-matrix hit cases but translated genuine substring allocation failure at ordinal 950 into incorrect X1 `unsupported`; its cold post-staging census found 31 allocations and a real operator-new failure aborted. The focused source repairs `08a6d9a` (X1) and `f604e87` (P1/TR3) preserve O2 source byte-for-byte. A focused O0 diagnostic/relink passed ordinal 949/950 failure and same-tuple retry, handler failure/retry, and cold plus repeated zero-allocation post-staging census. The authenticated O0 full sweep on reviewed `8cf0421` then passed allocation ordinals 0–1052 but failed at 1053: D2 hid an authentic T1 fingerprint-copy allocation failure as `invalid-argument`. Bounded shared D2 repair `63756bd` now prechecks same-aggregate tokenizer admission and preserves typed `internal/tokenizer-fingerprint` for an admitted tokenizer; a focused exact ordinal-1053 failure/retry probe and a public D2 outer-boundary failure probe pass. A supported final81298-source D2 aggregate built from `63756bd` and its public O0 error/recovery executable passed 24 compiled checks with empty compile/run diagnostics. Independent review of this successor and canonical full O0/O2 prefix gates on the exact clean repaired tree remain pending; no full lease-failure acceptance is claimed. Full supported union CI, facade remapping/package localization, trainer/compositor integration, public trainer, joint restore, one-batch overfit, held-out improvement and interrupted/resumed equivalence remain pending | active |
