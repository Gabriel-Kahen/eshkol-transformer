...
| C2 | [Detached full training-state checkpoint schema](C2_TRAINING_STATE.md) | C1, D2, O2, X1, K2 | K2 and all component prerequisites are merged. Private parser/reader/inspect, exact-bit staging, canonical deep-owner lifetime, staged LOAD/reconstruction, atomic SAVE, the fixed 16 MiB/512 KiB/8 MiB/64 tuple, two fresh measured joint runs below 524,288 KiB RSS, deviation modes, affected regressions, and independent lifetime review pass on the current development host. Real K2 public composition, the final public boundary, supported Ubuntu 22.04/LLVM 21 CI, and independent integration acceptance remain outstanding. Component continuation and atomic file publication only; TR3 retains joint live restore/full trajectory and G3 generation proof | active |
