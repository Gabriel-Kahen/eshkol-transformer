...
evidence="${1:-$(project_build_dir)/g3c4-manual-ao-test}"
mkdir -p "$evidence"
python3 "${PROJECT_ROOT}/scripts/check-g3c4-manual-ao.py" >"$evidence/static.stdout"
for manifest in \
...
if [[ "${G3C4_SKIP_AO_PREDECESSOR:-0}" == 1 ]]; then
  printf 'G3-C4 AT regression: SKIPPED\n' >"$evidence/at-regression.stdout"
else
  CC="$cc" "${PROJECT_ROOT}/scripts/test-g3c4-manual-at.sh" \
    "$evidence/at-regression" >"$evidence/at-regression.stdout" 2>&1
fi
...
"$cc" "${flags[@]}" -O2 "${base_macros[@]}" \
  ...
  -DET_G3C4_MANUAL_ROLE0_PRIVATE -DET_G3C4_MANUAL_PRE_A2_PRIVATE -DET_G3C4_MANUAL_A2_PRIVATE -DET_G3C4_MANUAL_AT_PRIVATE -c \
  "${PROJECT_ROOT}/src/eshkol_transformer/g3c4_model_owner.c" -o "$evidence/at-owner.o"
"$cc" "${flags[@]}" -O2 "${base_macros[@]}" \
  -DET_G3C4_LOGITS_RESERVATION_PRIVATE -DET_G3C4_MANUAL_FRAME_BEGIN_PRIVATE \
  ...
  "${PROJECT_ROOT}/src/eshkol_transformer/g3c4_model_owner.c" -o "$evidence/manual-ao-owner.o"
for object in at-owner manual-ao-owner; do
  nm -g --defined-only --format=posix "$evidence/$object.o" |
    awk 'NF >= 2 { print $1 }' | LC_ALL=C sort -u >"$evidence/$object-defined.txt"
done
comm -13 "$evidence/at-owner-defined.txt" "$evidence/manual-ao-owner-defined.txt" \
  >"$evidence/added-defined.txt"
test ! -s "$evidence/added-defined.txt"
if "$cc" "${flags[@]}" -O2 -DET_G3C4_MANUAL_AO_PRIVATE -c \
   "${PROJECT_ROOT}/src/eshkol_transformer/g3c4_model_owner.c" \
   -o "$evidence/invalid-ao-only.o" \
   >"$evidence/invalid-ao-only.stdout" 2>"$evidence/invalid-ao-only.stderr"; then
  printf 'invalid ao-only macro tuple compiled\n' >&2; exit 1
fi
grep -F 'ET_G3C4_MANUAL_AO_PRIVATE requires manual attention merge' \
  "$evidence/invalid-ao-only.stderr" >/dev/null
native_sources=(
  "${PROJECT_ROOT}/tests/g3c4/test_manual_ao.c"
...
compile_mode() {
  ...
  "$cc" "${flags[@]}" "${mode_flags[@]}" \
    ...
    -o "$evidence/$mode-manual-ao"
  "${runtime[@]}" timeout --foreground --signal=TERM --kill-after=5s 120s \
    "$evidence/$mode-manual-ao" \
    >"$evidence/$mode.stdout" 2>"$evidence/$mode.stderr"
  test ! -s "$evidence/$mode.stderr"
  grep -E '^G3-C4 internal manual AO PASS: checks=[1-9][0-9]* routes=3 ordinal=12$' \
    "$evidence/$mode.stdout" >/dev/null
}
compile_mode normal
"$evidence/normal-manual-ao" >"$evidence/runtime-repeat.stdout" \
...
sha256sum "${PROJECT_ROOT}/native/g3c4_manual_ao_source_closure.txt" \
  >"$evidence/closure.sha256"
cat "$evidence/static.stdout"
cat "$evidence/normal.stdout"
printf 'G3-C4 private manual AO evidence: %s\n' "$evidence"
