...
evidence="${1:-$(project_build_dir)/g3t-prefill-sample}"
mkdir -p "$evidence"
...
temporary="$(mktemp -d "${TMPDIR:-/tmp}/g3t-prefill-sample.XXXXXX")"
trap 'status=$?; if (( status == 0 )); then rm -rf -- "$temporary"; else echo "G3-T prefill/sample build retained: $temporary" >&2; fi' EXIT
...
build_mode() {
  ...
  ar rcsD "$directory/libg3t_prefill_sample.a" "$directory"/objects/*.o
...
WRAPPER
  ...
  env -u ESHKOL_PATH -u ESHKOL_JIT_CACHE_DIR ESHKOL_JIT_CACHE=0 \
    ...
    -L "$directory" --lib g3t_prefill_sample \
    "$PROJECT_ROOT/tests/g3t/prefill_sample_test.esk" -o "$directory/caller" \
  ...
  grep -E '^G3-T private prefill/sample PASS: checks=[1-9][0-9]*$' \
...
sha256sum "$PROJECT_ROOT/native/g3t_prefill_sample_source_closure.txt" >"$evidence/closure.sha256"
cat "$evidence/static.stdout"
cat "$evidence/normal.stdout"
printf 'G3-T prefill/sample evidence: %s\n' "$evidence"
