...
evidence="${1:-$(project_build_dir)/g3c4-manual-role0-test}"
mkdir -p "$evidence"
python3 "${PROJECT_ROOT}/scripts/check-g3c4-manual-role0.py" >"$evidence/static.stdout"
...
if [[ "${G3C4_SKIP_ROLE0_PREDECESSOR:-0}" == 1 ]]; then
  printf 'G3-C4 frame begin regression: SKIPPED\n' >"$evidence/frame-begin-regression.stdout"
else
  CC="$cc" "${PROJECT_ROOT}/scripts/test-g3c4-manual-frame-begin.sh" \
    "$evidence/frame-begin-regression" >"$evidence/frame-begin-regression.stdout" 2>&1
fi
...
"$cc" "${flags[@]}" -O2 "${base_macros[@]}" \
  ...
  -DET_G3C4_MANUAL_ROLE0_PRIVATE -c \
  "${PROJECT_ROOT}/src/eshkol_transformer/g3c4_model_owner.c" -o "$evidence/role0-owner.o"
for object in base-owner role0-owner; do
  nm -g --defined-only --format=posix "$evidence/$object.o" |
    awk 'NF >= 2 { print $1 }' | LC_ALL=C sort -u >"$evidence/$object-defined.txt"
done
comm -13 "$evidence/base-owner-defined.txt" "$evidence/role0-owner-defined.txt" \
  >"$evidence/added-defined.txt"
test ! -s "$evidence/added-defined.txt"
if "$cc" "${flags[@]}" -O2 -DET_G3C4_MANUAL_ROLE0_PRIVATE -c \
   "${PROJECT_ROOT}/src/eshkol_transformer/g3c4_model_owner.c" \
   -o "$evidence/invalid-role0-only.o" \
   >"$evidence/invalid-role0-only.stdout" 2>"$evidence/invalid-role0-only.stderr"; then
  printf 'invalid role0-only macro tuple compiled\n' >&2; exit 1
fi
grep -F 'ET_G3C4_MANUAL_ROLE0_PRIVATE requires manual frame and token provider routes' \
  "$evidence/invalid-role0-only.stderr" >/dev/null
native_sources=(
  "${PROJECT_ROOT}/tests/g3c4/test_manual_role0.c"
...
compile_mode() {
  ...
  "$cc" "${flags[@]}" "${mode_flags[@]}" \
    ...
    -o "$evidence/$mode-manual-role0"
  "${runtime[@]}" timeout --foreground --signal=TERM --kill-after=5s 120s \
    "$evidence/$mode-manual-role0" \
    >"$evidence/$mode.stdout" 2>"$evidence/$mode.stderr"
  test ! -s "$evidence/$mode.stderr"
  grep -E '^G3-C4 internal manual role0 PASS: checks=[1-9][0-9]* routes=3 failure-retry=1$' \
    "$evidence/$mode.stdout" >/dev/null
}
compile_mode normal
"$evidence/normal-manual-role0" >"$evidence/runtime-repeat.stdout" \
...
sha256sum "${PROJECT_ROOT}/native/g3c4_manual_role0_source_closure.txt" \
  >"$evidence/closure.sha256"
cat "$evidence/static.stdout"
cat "$evidence/normal.stdout"
printf 'G3-C4 private manual role0 evidence: %s\n' "$evidence"
