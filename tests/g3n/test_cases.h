#ifndef ET_G3N_ADVERSARIAL_CASES_H
#define ET_G3N_ADVERSARIAL_CASES_H
...
/* Rehouse the independently enumerated fixture in guarded, uniformly aligned
 * storage. This allows byte snapshots to include every operand and descriptor. */
static inline void setup(fixture *f,const et_kernel_capability_v1 *cap,size_t op,size_t row) {
  g3n_fixture src;size_t found=G3N_ROWS;
  for(size_t i=0;i<G3N_ROWS;++i) {
    fixture_init(&src,i);
  ...
  CHECK(found<G3N_ROWS);memset(f,0,sizeof(*f));memset(f->ob,0xa5,sizeof(f->ob));
  memcpy(f->row,src.shape,sizeof(f->row));f->r=src.request;f->r.shape=f->row;
  for(size_t i=0;i<src.call.input_count;++i) {
    memcpy(f->ish[i],src.input_shapes[i],sizeof(f->ish[i]));
    f->in[i]=src.inputs[i];memcpy(&f->ib[i],src.inputs[i].data,src.inputs[i].byte_length);
    f->in[i].data=&f->ib[i];f->in[i].shape=f->ish[i];
  }
  memcpy(f->osh[0],src.output_shape,sizeof(f->osh[0]));f->out[0]=src.outputs[0];f->out[0].data=&f->ob[0];f->out[0].shape=f->osh[0];
