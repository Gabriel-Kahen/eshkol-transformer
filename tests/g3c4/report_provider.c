...
#ifndef G3C4_REPORT_ACCESSOR
#define G3C4_REPORT_ACCESSOR et_g3c4_kernel_provider_v1
#endif
extern const et_kernel_provider_v1 *G3C4_REPORT_ACCESSOR(void);

static const et_kernel_provider_v1 *resolve(void *context, const char *symbol) {
  (void)context;
  (void)symbol;
  return G3C4_REPORT_ACCESSOR();
}
...
int main(void) {
  ...
  if (report == NULL ||
      et_kernel_runtime_report_json(runtime, report, required, &required, &error) != 0) {
    fprintf(stderr, "error: cannot create provider report\n");
    free(report);
    et_kernel_runtime_destroy(runtime);
    return 1;
  }
  if (fwrite(report, 1u, required - 1u, stdout) != required - 1u) {
    fprintf(stderr, "error: cannot write provider report\n");
    free(report);
    et_kernel_runtime_destroy(runtime);
    return 1;
  }
