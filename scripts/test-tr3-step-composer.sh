...
[[ -z "$(git status --porcelain --untracked-files=all)" ]] || \
  die "step composer gate requires a clean committed checkout"
evidence="${TR3_STEP_COMPOSER_EVIDENCE_DIR:-$(mktemp -d "${TMPDIR:-/tmp}/tr3-step-composer.XXXXXX")}"
mkdir -p -- "${evidence}"
...
def visit(path):
    ...
    for name in re.findall(r'\(load\s+"([^"]+)"\)', path.read_text()):
        ...
        visit(found)
visit(root / 'tests/tr3_step_composer/runtime.esk')
paths = sorted(str(path.relative_to(root)) for path in seen)
for required in ('native/tr3_step_composer_root.esk',
                 'native/tr3_step_composer_extension.esk',
                 'native/tr3_step_commit_tail_root.esk',
...
observations = [line.replace('TR3-STEP-COMPOSER-OBS', 'TR3-STEP-OBS')
...
printf 'TR3 step composer evidence: %s\n' "${evidence}"
printf 'TR3 step composer seal: %s\n' \
