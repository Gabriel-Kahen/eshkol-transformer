...
evidence="${TR3_PUBLIC_RELEASE_CANDIDATE_EVIDENCE_DIR:-$(mktemp -d "${TMPDIR:-/tmp}/tr3-public-release-candidate.XXXXXX")}"
mkdir -p -- "${evidence}"
...
/usr/bin/time -v -o /out/source-compile.time \
  ...
  /fixed/eshkol-run-release --strict-types --no-stdlib -O 0 \
    ...
    native/tr3_public_release_candidate_root.esk -o /out/private \
...
objcopy --redefine-syms=native/tr3_public_release_candidate_private_renames.txt \
  /out/private.o
...
clang-21 -std=c11 -Wall -Wextra -Werror -Wpedantic -fPIC \
  -I /fixed-source/inc -I native -MMD -MF /out/native/tr3_public_release_candidate_bridge.o.d \
  -c native/tr3_public_release_candidate_bridge.c \
  -o /out/native/tr3_public_release_candidate_bridge.o
...
awk "NR == FNR {allowed[\$1] = 1; next} !(\$1 in allowed) {print \$1}" \
  native/tr3_public_release_candidate_exports.txt /out/raw-defined.txt \
...
cmp native/tr3_public_release_candidate_exports.txt /out/global-defined.txt
nm -u --format=posix /out/combined.o | awk "{print \$1}" | \
  LC_ALL=C sort -u > /out/undefined-symbols.txt
ar rcsD /out/libeshkol_transformer_tr3_public_release_candidate.a /out/combined.o
test "$(ar t /out/libeshkol_transformer_tr3_public_release_candidate.a)" = combined.o
...
timeout --foreground --signal=TERM --kill-after=5s 120s \
  /fixed/eshkol-run-release --strict-types --no-stdlib -O 0 \
    ...
    tests/tr3_public_release_candidate/runtime.esk -o /out/caller.o \
    > /out/caller-check.stdout 2> /out/caller-check.stderr
timeout --foreground --signal=TERM --kill-after=5s 120s \
  /fixed/eshkol-run-release --strict-types --no-stdlib -O 0 \
    -I lib -L /out -L /candidate/eshkol-build-canonical \
    --lib eshkol_transformer_tr3_public_release_candidate \
    tests/tr3_public_release_candidate/runtime.esk \
...
grep -Fx TR3-PUBLIC-RELEASE-CANDIDATE-PASS /out/caller.stdout >/dev/null
...
PY
cmp native/tr3_public_release_candidate_source_closure.txt "${evidence}/source-closure.txt"
...
PY
cmp native/tr3_public_release_candidate_native_source_closure.txt \
  "${evidence}/native-source-closure.txt"
cmp native/tr3_public_release_candidate_native_objects.txt \
  "${evidence}/native-objects.txt"
cmp native/tr3_public_release_candidate_undefined_symbols.txt \
...
dep = Path(sys.argv[1]).read_text().replace('\\\n', ' ')
paths = [path.removeprefix('/workspace/') for path in dep.split(':', 1)[1].split()]
assert paths == ['tests/tr3_public_release_candidate/runtime.esk',
                 'lib/transformer/error_consumer.esk'], paths
PY
if rg '^et_e1b_private_|^tr3-lease-create-internal$|^tr3-lease-unenroll-internal!$|^trainer-create$|^trainer-release!$' \
...
find "${evidence}" -type f ! -name SHA256SUMS -printf '%P\n' | \
  ...
  >"${evidence}/SHA256SUMS"
printf 'TR3 public release candidate linked PASS: %s\n' "${evidence}"
