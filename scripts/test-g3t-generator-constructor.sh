...
evidence="${1:-$(project_build_dir)/g3t-generator-constructor}"
mkdir -p "$evidence"
...
temporary="$(mktemp -d "${TMPDIR:-/tmp}/g3t-generator-constructor.XXXXXX")"
trap 'status=$?; if (( status == 0 )); then rm -rf -- "$temporary"; else echo "G3-T generator constructor build retained: $temporary" >&2; fi' EXIT
...
build_mode() {
  ...
  ar rcsD "$directory/libg3t_generator_constructor.a" "$directory"/objects/*.o
...
WRAPPER
  ...
  env -u ESHKOL_PATH -u ESHKOL_JIT_CACHE_DIR ESHKOL_JIT_CACHE=0 \
    ...
    -L "$directory" --lib g3t_generator_constructor \
    "$PROJECT_ROOT/tests/g3t/generator_constructor_test.esk" -o "$directory/caller" \
  ...
  grep -E '^G3-T private generator constructor PASS: checks=[1-9][0-9]*$' \
