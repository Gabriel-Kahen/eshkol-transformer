...
evidence="${1:-$(project_build_dir)/g3c4-prefill2-test}"
mkdir -p "${evidence}"

python3 "${PROJECT_ROOT}/scripts/check-g3c4-prefill2.py" \
...
prefill1_macros=(
  ...
  -DET_G3C4_PREFILL1_PRIVATE -DET_A2_KV_CACHE_STORAGE_QUERY_PRIVATE
)
prefill2_macros=("${prefill1_macros[@]}" -DET_G3C4_PREFILL2_PRIVATE)

"${cc}" "${flags[@]}" -O2 "${prefill1_macros[@]}" -c \
  "${PROJECT_ROOT}/src/eshkol_transformer/g3c4_model_owner.c" \
  -o "${evidence}/prefill1-owner.o"
"${cc}" "${flags[@]}" -O2 "${prefill2_macros[@]}" -MMD \
  -MF "${evidence}/prefill2.d" -c \
  "${PROJECT_ROOT}/src/eshkol_transformer/g3c4_model_owner.c" \
  -o "${evidence}/prefill2-owner.o"
for object in prefill1-owner prefill2-owner; do
...
comm -13 "${evidence}/prefill1-owner-defined.txt" \
  "${evidence}/prefill2-owner-defined.txt" \
  >"${evidence}/added-defined.txt"
printf '%s\n' et_g3c4_private_prefill2_v1 |
  cmp - "${evidence}/added-defined.txt"
comm -13 "${evidence}/prefill1-owner-undefined.txt" \
  "${evidence}/prefill2-owner-undefined.txt" \
  >"${evidence}/added-undefined.txt"
test ! -s "${evidence}/added-undefined.txt"

if "${cc}" "${flags[@]}" -O2 \
    -DET_G3C4_PREFILL2_PRIVATE -c \
    "${PROJECT_ROOT}/src/eshkol_transformer/g3c4_model_owner.c" \
    -o "${evidence}/invalid-prefill2-only.o" \
    >"${evidence}/invalid-prefill2-only.stdout" \
    2>"${evidence}/invalid-prefill2-only.stderr"; then
  printf 'invalid P2-prefill-only macro tuple compiled\n' >&2
  exit 1
fi
grep -F 'ET_G3C4_PREFILL2_PRIVATE requires Step 16A' \
  "${evidence}/invalid-prefill2-only.stderr" >/dev/null

sources=(
  "${PROJECT_ROOT}/tests/g3c4/test_prefill2.c"
...
for mode in normal sanitize; do
  ...
  "${cc}" "${flags[@]}" "${mode_flags[@]}" -DET_A2_KV_CACHE_TESTING \
    ...
    -o "${evidence}/prefill2-${mode}"
  "${environment[@]}" "${evidence}/prefill2-${mode}" \
    >"${evidence}/prefill2-${mode}.stdout" \
    2>"${evidence}/prefill2-${mode}.stderr"
  test ! -s "${evidence}/prefill2-${mode}.stderr"
done
"${evidence}/prefill2-normal" \
  >"${evidence}/prefill2-repeat.stdout" \
  2>"${evidence}/prefill2-repeat.stderr"
test ! -s "${evidence}/prefill2-repeat.stderr"
cmp "${evidence}/prefill2-normal.stdout" \
  "${evidence}/prefill2-repeat.stdout"
cmp "${evidence}/prefill2-normal.stdout" \
  "${evidence}/prefill2-sanitize.stdout"
grep -E '^G3-C4 prefill2 PASS: checks=[1-9][0-9]* roles=21 dispatch-cuts=21 a2-allocation-cuts=11 runtime-allocation-cuts=2$' \
  "${evidence}/prefill2-normal.stdout" >/dev/null

git -C "${PROJECT_ROOT}" diff --check
sha256sum "${PROJECT_ROOT}/native/g3c4_prefill2_source_closure.txt" \
  >"${evidence}/closure.sha256"
cat "${evidence}/static.stdout"
cat "${evidence}/prefill2-normal.stdout"
printf 'G3-C4 private P2 prefill evidence: %s\n' "${evidence}"
