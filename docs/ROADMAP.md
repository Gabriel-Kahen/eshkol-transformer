...
| E3 | [Loss, perplexity, token accuracy and validation runner](E3_EVALUATION_PROPOSAL.md) | M3/M3T, D2, L2/L3S; shared guard and bool metric seam accepted; E3 frame and P1/D2 restore pending | #99; the bounded shared-call implementation, E3-METRICS and E3-D2 prerequisite union was accepted and merged as `33a54ef` after supported 19-suite/29-command CI and focused merged-head checks. The exact nine-file deterministic reference/oracle toolkit from candidate `37a8078` is carried here without CI registration. Explicit private consumer dispatch, frame restoration, staging/rollback, runtime/public evaluation, optional weighted paths and full evaluator acceptance remain pending | active |
| E3-D2 | [Canonical-byte identity and native idle prerequisite](e3/E3_D2_PREREQUISITE.md) | T1, D1, D2; merged PR #104 contract | #106; exact one-form hash-pinned source variant, authentic T1 admission and native idle 0/1/2; private compiled closure, lifecycle, provenance and sanitizer gates; byte-preserving prerequisite union passed supported CI 35744832879 and merged as `33a54ef7`; focused merged-head gates passed. Frame-bound restoration, staging, rollback and full E3 composition remain deferred | complete |
| G3 | Greedy, temperature, top-k/top-p generation with KV cache | M3, T1, A2 | #89 [bounded design and compiled reachability](G3_GENERATION_PROPOSAL.md), decision 5748532295; G3-N is accepted and merged, while G3-S and the exact G3-T private-seam implementation remain dependencies. This candidate also carries the bounded G3-C4-N provider, but no C4 model, sampler, transport, KV-cache continuation or end-to-end generation/save-reload acceptance is claimed | active |
...
| G3-C4-N | [Exact C4 forward numerical provider](G3C4_PRIMITIVES.md) | K1, I1, I2; G3-N, N2, N3K, A2 provenance | [#115](https://github.com/Gabriel-Kahen/eshkol-transformer/issues/115); accepted ABI 1.0 with seven capabilities/eight operations/28 exact pairs; standalone implementation has independent approval and complete local LLVM22 compatibility evidence. Exact integration source `637cec0` and the combined `2006cde` base passed independent union review; its supported C4 numerical/ABI/two-AOT/sanitizer suite has passed in full run 35799381054 attempt 2, while the remaining full-run jobs and root acceptance are pending. No C4 model, sampler, transport or generation acceptance is claimed | review |
