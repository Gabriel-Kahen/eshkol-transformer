...
evidence="${1:-${PROJECT_ROOT}/build/g3c4-output-decode-ids-test}"
cc="${CC:-clang}"
mkdir -p "${evidence}"
...
python3 "${PROJECT_ROOT}/scripts/check-g3c4-output-decode-ids.py" \
...
decode_macros=("${base_macros[@]}" -DET_G3C4_OUTPUT_DECODE_IDS_PRIVATE)

"${cc}" "${flags[@]}" -O2 "${base_macros[@]}" -c \
  "${PROJECT_ROOT}/src/eshkol_transformer/g3c4_model_owner.c" \
  -o "${evidence}/base-owner.o"
"${cc}" "${flags[@]}" -O2 "${decode_macros[@]}" -MMD \
  -MF "${evidence}/decode.d" -c \
  "${PROJECT_ROOT}/src/eshkol_transformer/g3c4_model_owner.c" \
  -o "${evidence}/decode-owner.o"
for object in base-owner decode-owner; do
...
comm -13 "${evidence}/base-owner-defined.txt" \
  "${evidence}/decode-owner-defined.txt" \
  >"${evidence}/added-defined.txt"
printf '%s\n' et_g3c4_private_output_copy_decode_ids_v1 |
  LC_ALL=C sort | cmp - "${evidence}/added-defined.txt"
comm -13 "${evidence}/base-owner-undefined.txt" \
  "${evidence}/decode-owner-undefined.txt" \
  >"${evidence}/added-undefined.txt"
printf '%s\n' et_i64_tensor_private_storage_overlap_v1 |
  LC_ALL=C sort >"${evidence}/expected-added-undefined.txt"
cmp "${evidence}/expected-added-undefined.txt" \
  "${evidence}/added-undefined.txt"

"${cc}" "${flags[@]}" -O2 -c \
  "${PROJECT_ROOT}/native/i64_tensor.c" \
  -o "${evidence}/base-i1.o"
"${cc}" "${flags[@]}" -O2 \
  -DET_I64_TENSOR_STORAGE_QUERY_PRIVATE -c \
  "${PROJECT_ROOT}/native/i64_tensor.c" \
  -o "${evidence}/private-i1.o"
for object in base-i1 private-i1; do
  nm -g --defined-only --format=posix "${evidence}/${object}.o" |
    awk 'NF >= 2 { print $1 }' | LC_ALL=C sort -u \
    >"${evidence}/${object}-defined.txt"
  nm -u --format=posix "${evidence}/${object}.o" |
...
comm -13 "${evidence}/base-i1-defined.txt" \
  "${evidence}/private-i1-defined.txt" \
  >"${evidence}/i1-added-defined.txt"
cmp "${evidence}/expected-added-undefined.txt" \
  "${evidence}/i1-added-defined.txt"
comm -13 "${evidence}/base-i1-undefined.txt" \
  "${evidence}/private-i1-undefined.txt" \
  >"${evidence}/i1-added-undefined.txt"
test ! -s "${evidence}/i1-added-undefined.txt"

if "${cc}" "${flags[@]}" -O2 \
    -DET_G3C4_OUTPUT_DECODE_IDS_PRIVATE -c \
    "${PROJECT_ROOT}/src/eshkol_transformer/g3c4_model_owner.c" \
    -o "${evidence}/invalid-decode-only.o" \
    >"${evidence}/invalid-decode-only.stdout" \
    2>"${evidence}/invalid-decode-only.stderr"; then
  printf 'invalid decode-ID-only macro tuple compiled\n' >&2
  exit 1
fi
grep -F 'ET_G3C4_OUTPUT_DECODE_IDS_PRIVATE requires Step 20A and the private I1 storage query' \
  "${evidence}/invalid-decode-only.stderr" >/dev/null

sources=(
  "${PROJECT_ROOT}/tests/g3c4/test_output_decode_ids.c"
...
compile_mode() {
  ...
  "${cc}" "${flags[@]}" "${mode_flags[@]}" \
    ...
    -o "${evidence}/output-decode-ids-${mode}"
  "${environment[@]}" "${evidence}/output-decode-ids-${mode}" \
    >"${evidence}/${mode}.stdout" 2>"${evidence}/${mode}.stderr"
  test ! -s "${evidence}/${mode}.stderr"
}

compile_mode normal
"${evidence}/output-decode-ids-normal" \
...
grep -E '^G3-C4 output decode IDs PASS: checks=[1-9][0-9]* routes=2 borrow-allocation-cuts=1 active-borrow-cuts=1 alias-cuts=6 ownership-cuts=3 readiness-cuts=2 malformed-cuts=5 repeat-cuts=1$' \
  "${evidence}/normal.stdout" >/dev/null

git -C "${PROJECT_ROOT}" diff --check
sha256sum "${PROJECT_ROOT}/native/g3c4_output_decode_ids_source_closure.txt" \
  >"${evidence}/closure.sha256"
cat "${evidence}/static.stdout"
cat "${evidence}/normal.stdout"
printf 'G3-C4 private output decode-ID evidence: %s\n' "${evidence}"
