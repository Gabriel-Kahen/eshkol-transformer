...
check_public_module transformer/module \
  'module-parameters module-buffers module-state-dict module-load-state-dict! module-train! module-eval! module-zero-grad! parameter-tree-paths parameter-tree-handle parameter-tree-tie-groups parameter-handle-path parameter-handle-shape parameter-handle-dtype parameter-handle-device state-dict-paths state-dict-tensor state-dict-alias-groups'
...
[[ "$(wc -l <"${t1_boundary_evidence}/global-defined.txt")" == 46 ]] || \
  die "Wave 1 aggregate must expose E1 6 + P1 17 + D1 8 + X1 6 + C1 1 + T1 8"
[[ "$(wc -l <"${t1_boundary_evidence}/package-exports.txt")" == 40 ]] || \
  die "Wave 1 package export manifest must contain 40 non-E1 wrappers"
...
for private_binding in "${private_source_bindings[@]}"; do
  # The first probe starts from an empty cache; later names deliberately reuse
  # that public-module cache so this exhaustive name check stays a focused gate.
...
if strings -a "${t1_boundary_tmp}/import-reverse.o" | \
    grep -E 'et-e1b-private-raise|t1-private-|t1-wave1-|c1-persistence-policy-internal|x1-private-|p1-native-|d1-token-corpus-(write|validate)-impl' \
      >/dev/null; then
  die "public aggregate caller contains a private source binding"
fi
...
cmp "${PROJECT_ROOT}/native/t1_wave1_defined_symbols.txt" \
  "${t1_boundary_tmp}/public-wrapper-refs.txt" || \
  die "aggregate public source closure does not reference exactly 46 wrappers"
...
for deterministic_evidence in global-defined.txt package-exports.txt \
    undefined.txt expected-undefined.txt private.d link.map; do
...
printf 'T1 BOUNDARY PASS: exact 46-global source/localization/module manifests, reverse imports, hostile-path and copied/prelocalized/standalone rejection, crafted links, cross-artifact collision, and Python isolation\n'
