...
def validate(installed, source, depfile):
    ...
    facades = (ROOT / "native/m3t_package_facades.txt").read_text().splitlines()
    ...
    if set(dependencies) != allowed:
        raise ValueError("public depfile must contain the exact caller and all six facades")
    return dependencies
...
if __name__ == "__main__":
    try:
        ...
        raise SystemExit(f"M3T public source closure rejected: {error}")
    print(f"M3T installed public source closure PASS: {len(dependencies)} files")
